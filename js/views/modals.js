...
<sc-if value="{{ staffEditing }}">
  <div style="position:fixed;inset:0;background:rgba(42,51,44,.5);backdrop-filter:blur(6px);display:grid;place-items:center;z-index:50;">
    <div style="background:rgba(251,248,241,.92);backdrop-filter:blur(14px);border:1px solid rgba(255,255,255,.7);border-radius:18px;padding:30px;width:400px;box-shadow:0 24px 60px rgba(0,0,0,.3);">
      <div style="font-family:'Marcellus',serif;font-size:22px;color:#2E4A3C;margin-bottom:18px;">Edit staff</div>
      <div style="font-size:12.5px;letter-spacing:1px;color:#7A8378;margin-bottom:6px;">NAME</div>
      <input value="{{ editStaffName }}" onChange="{{ setEditStaffName }}" style="width:100%;padding:10px;border:1px solid #E3DCCB;background:#F4EEE3;font-size:14px;margin-bottom:14px;outline:none;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px;">
        <div>
          <div style="font-size:12.5px;letter-spacing:1px;color:#7A8378;margin-bottom:6px;">STAFF ID</div>
          <input value="{{ editStaffId }}" onChange="{{ setEditStaffId }}" style="width:100%;padding:10px;border:1px solid #E3DCCB;background:#F4EEE3;font-size:14px;outline:none;">
        </div>
        <div>
          <div style="font-size:12.5px;letter-spacing:1px;color:#7A8378;margin-bottom:6px;">ROLE</div>
          <select value="{{ editStaffRole }}" onChange="{{ setEditStaffRole }}" style="width:100%;padding:10px;border:1px solid #E3DCCB;background:#F4EEE3;font-size:14px;">
            <option>Manager</option><option>Cashier</option><option>Barista</option>
          </select>
        </div>
      </div>
      <div style="display:flex;gap:10px;justify-content:flex-end;">
        <button onClick="{{ closeStaffEdit }}" style="border:1px solid #E3DCCB;background:#FBF8F1;padding:10px 18px;cursor:pointer;font-size:14px;">Cancel</button>
        <button onClick="{{ saveStaff }}" style="background:#C1744E;color:#FBF8F1;border:none;padding:10px 22px;cursor:pointer;font-size:14px;" style-hover="background:#A85F3D;">Save</button>
      </div>
    </div>
  </div>
</sc-if>
